    ...
    class Config:
        json_schema_extra = {
            "example":
                {
                    "total_orders": 50,
                    "avg_order_value": 850.60,
                    "avg_quantity_per_order": 25.3
                }
        }

app = FastAPI(title="Country Sales Pattern API")

# Load trained model (scaler and DBSCAN) from file
model_data = joblib.load("model4.pkl")
...
@app.post("/predict")
def predict(items: List[CountryData]):
    # Prepare feature array
    X = np.array([[item.total_orders, item.avg_order_value, item.avg_quantity_per_order] for item in items])
    # Scale features
